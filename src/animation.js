define('niagara-animation', function(require) {
	...
	// TODO:
	// function smoothInterpolator(t, start, end, startVelocity, endVelocity);
	...
	// - supports multi-property animation (like 'rotate(0 30 30)' to 'rotate(45 100 100)')

	function extractNumbers(s) {
		var nums = [], m;
		while (m = /-?[0-9]+(?:\.[0-9]+)?/g.exec(s)) {
			nums.push(parseFloat(m[0]));
		}
		return nums;
	}

	function setNumbers(s, nums) {
		var i = 0;
	...
	function dial(properties1, properties2, velocity1, velocity2) {
		var self = this;
		var v1 = velocity1, v2 = velocity2;
		if (!v2)
			v2 = v1; 
		...
		return function(t) {
			eachObj(properties1, function(name, start) {
				var vStart = (v1 && v1[name]) || 0;
				var vEnd = (v2 && v2[name]) || 0;
				...
				self.set(name, t<=0?start:t>=1?end: 
					 (/^#|rgb\(/.test(end)) ? // color in format '#rgb' or '#rrggbb' or 'rgb(r,g,b)'?
							    ...
							    + ')')
							:   // TODO: multi-prop
								replace(end, /-?[\d.]+/, toString(smoothInterpolator(extractNumber(start), extractNumber(end), t, vStart, vEnd)))
				);
			});
		};
	},
	...
	// TODO
	// list.smoothAnimate({state1:0}, {state2:0}, {velocity1:0}, {velocity2: 0}, duration)
	// - uses smoothDial()

	// TODO
	// timeline-based creation of interpolation functions, using smoothAnimator. 
	...
	function createInterpolator(listOfKeyFrames) {
	     ...
	     var fGen =  new Function("start", "d", "t", "subInterpolators", 
	          ...
	          _(keyFrames).sub(1).map(function(kf, i) {
	               return ' else if (t < '+(kf.t)+') return start+d*subInterpolators['+(i)+'];\n';
	          }).join('');
	     );

	     var smoothInterpolators = []; // TODO: create funcs
	...
	define('animation', function() {
	     ...
	     function create(timelineDescriptor) {
	          function processItem(tStart, e) {
	               if (_.isList(e)) {
	                    var ll = _.map(e, function(li) { processItem(tStart, li});
	                    return [
